...
func (webauthn *WebAuthn) validateLogin(user User, session SessionData, parsedResponse *protocol.ParsedCredentialAssertionData) (*Credential, error) {
	...
	if webauthn.Config.MDS != nil {
		...
		if e := protocol.ValidateCredentialRecordMetadata(context.Background(), webauthn.Config.MDS, aaguid, credential.AttestationType, credential.AttestationFormat, credential.attestationX5C()); e != nil {
