...
func attestationFormatValidationHandlerCompound(att AttestationObject, clientDataHash []byte, mds metadata.Provider) (attestationType string, x5cs []any, err error) {
	...
	for _, attStmt := range attStmts {
		...
		handler, valid := attestationRegistry[AttestationFormat(object.Format)]

		if !valid {
			return "", nil, ErrAttestationFormat.WithInfo(fmt.Sprintf("Attestation sub-statement format %s is unsupported", object.Format))
		}

		var cx5cs []any
		if _, cx5cs, err = handler(object, clientDataHash, mds); err != nil {
			return "", nil, err
		}
